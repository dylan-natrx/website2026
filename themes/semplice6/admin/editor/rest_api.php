...
class editor_api extends editor {
	...
	public function __construct() {
		...
		// modules list
		$modules_list = array('advancedportfoliogrid', 'blogarchives', 'blogposts', 'blogsearch', 'blogcomments', 'button', 'code', 'dribbble', 'gallery', 'image', 'lottie', 'oembed', 'paragraph', 'portfoliogrid', 'share', 'singleproject', 'socialprofiles', 'spacer', 'text', 'fluidtext', 'video', 'marquee', 'accordion', 'instagram', 'gallerygrid', 'mailchimp', 'beforeafter');

		// include files
		foreach ($modules_list as $module) {
			require get_template_directory() . '/admin/editor/modules/' . $module . '.php';
		}
	...
	public function register_routes() {
		...
		$routes = array(
			...
			'/save/template'		=> array('creatable', 'save_template'),
			...
			'/blog/posts'			=> array('creatable', 'blogposts'),
			'/blog/archives'		=> array('creatable', 'blogarchives'),
	...
	public function masonry($request) {
		...
		// output
		$output = editor_api::$modules[$request['module']]->output_editor($content, $request['id']);

		return new WP_REST_Response($output, 200);
	}

	// get blogposts
	public function blogposts($request) {
		...
		if(!empty($request['wp_template'])) {
			$content['posts_filter'] = array(
				'type' => $request['wp_template'],
				'meta' => false
			);
		} else {
			$content['posts_filter'] = false;
		}
		
		// output
		$output = editor_api::$modules['blogposts']->output_editor($content, $request['id']);

		return new WP_REST_Response($output, 200);
	}

	// get blogarchives
	public function blogarchives($request) {
		...
		// output
		$output = editor_api::$modules['blogarchives']->output_editor($content, $request['id']);
	...
	public function single_project($request) {
		...
		// output
		$output = editor_api::$modules['singleproject']->output_editor($content, $request['id']);
	...
	public function edit_apg($request) {
		...
		// output
		$output = editor_api::$modules['advancedportfoliogrid']->output_editor($content, $request['id']);
	...
	public function save_template($request) {
		...
		// save template
		update_option('semplice_template_' . $request['post_id'], $content);

		// save template settings
		update_option('semplice_template_' . $request['post_id']. '_settings', $request['post_settings']);
	...
	public function coverslider($request) {
		// return slider html
		return new WP_REST_Response(semplice_get_coverslider($request['covers'], 'editor', 'normal', false), 200);
	}
