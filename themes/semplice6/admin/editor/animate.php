	...
	class animate_api {
		...
		public function get_motion_output($output, $atts, $id, $mode, $post_id) {
			...
			if(true === $this->detect->isMobile() && $mobile == 'on' || false === $this->detect->isMobile()) {
				if(!empty($atts['motions']['timeline']) || true === $is_lottie || isset($atts['motions']['event']) && isset($atts['motions']['initial']) && $atts['motions']['event'] == 'on_scroll') {
					// get css
					if(false === $is_lottie) {
		...
		public function get_motion_css($mode, $atts, $motions, $id) {
			// vars
			$css = array('regular' => array(), 'transform' => array(), 'boxShadow' => array(), 'gradient' => array(), 'filter' => array(), 'reveal' => array());
			$output_css = '';
			$text_color_css = '';
			$transform = '';
			$box_shadow = '';
			...
			$css = $this->get_effects_css($css, $initial_defaults, $atts['styles']['xl']);
			// text color
			...
			// get reveal
			$reveal = $this->reveal($css['reveal'], $motions['initial']);
			// has reveal in steps
			$has_reveal = $this->has_reveal($motions);
			// only add if initial changed or steps with reveal
			$default_reveal = 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)';
			if(true === $has_reveal || $reveal != $default_reveal) {
				// add reveal to gsap
				if(false !== $reveal && !empty($reveal)) {
					$gsap_set['clipPath'] = $reveal;
					$output_css .= 'clip-path: ' . $reveal . ';';
				}
			}		
		...
		public function get_motion_js($atts, $id, $is_lottie) {
				...
				case 'on_scroll':
					...
					foreach ($options as $attr => $default_val) {
						if(isset($motions[$attr])) {
							...
							if(is_string($val) && $val == 'true' || is_string($val) && $val == 'false') {
			...
			// check if timeline is defined and add steps to timeline
		...
		public function gradient($gradient_values, $bg_image_url) {
			...
			if(!empty($gradient_values)) {
				...
				// check colors for transparency
				$colors = array('gradient_color_1', 'gradient_color_2');
				foreach($colors as $color) {
					if($defaults[$color] == 'transparent') {
						$defaults[$color . '_opacity'] = 0;
					}
				}
				// color and opacity
		...
		public function reveal($reveal_values, $initial) {
			if(!empty($reveal_values)) {
				$type = 'rectangle';
				$values = array('reveal_x1_hor' => 0, 'reveal_x1_ver' => 0, 'reveal_x2_hor' => 100, 'reveal_x2_ver' => 0, 'reveal_y1_hor' => 100, 'reveal_y1_ver' => 100, 'reveal_y2_hor' => 0, 'reveal_y2_ver' => 100);
				// is circle?
				if(isset($initial['reveal_type']) && $initial['reveal_type'] == 'circle') {
					$type = 'circle';
					$values = array('reveal_circle_size' => 50, 'reveal_circle_x' => 50, 'reveal_circle_y' => 50);
				}
				// iterate reveal values
				if(!empty($reveal_values)) {
					foreach ($reveal_values as $attribute => $value) {
						$values[$attribute] = $value;
					}
				}
				// check reveal type and return
				if($type == 'rectangle') {
					return 'polygon(' . $values['reveal_x1_hor'] . '% ' . $values['reveal_x1_ver'] . '%, ' . $values['reveal_x2_hor'] . '% ' . $values['reveal_x2_ver'] . '%, ' . $values['reveal_y1_hor'] . '% ' . $values['reveal_y1_ver'] . '%, ' . $values['reveal_y2_hor'] . '% ' . $values['reveal_y2_ver'] . '%)';
				} else {
					return 'circle(' . $values['reveal_circle_size'] . '% at ' . $values['reveal_circle_x'] . '% ' . $values['reveal_circle_y'] . '%)';
				}
			} else {
				return false;
			}
		...
		public function has_reveal($motions) {
			$has_reveal = false;
			// iterate steps
			foreach($motions['timeline'] as $steps => $step) {
				foreach($step['effects'] as $attribute => $value) {
					if(false !== strpos($attribute, 'reveal_')) {
						$has_reveal = true;
					}
				}
			}
		...
		public function get_effects_css($css, $effects, $styles) {
			// effects defaults
			$effects_defaults = $this->get_effects_defaults();
			// transform
			$transform = array('scaleX', 'scaleY', 'translateX', 'translateY', 'rotate', 'rotateX', 'rotateY', 'skewX', 'skewY');
			foreach ($effects as $effect => $value) {
				...
				if(in_array($effect, $transform)) {
					...
					$css['filter'][$effect] = $value;
				} else if(strpos($effect, 'reveal') !== false) {
					$css['reveal'][$effect] = $value;
				} else if(strpos($effect, 'border-') !== false) {
					$css['regular'][$effect] = $this->get_border_width($styles, $value);
		...
		public function get_border_width($styles, $val) {
			$width = $val;
		...
		public function get_animation_props($animation, $id, $atts, $step) {
			...
			$reveal_props = array();
			$module = isset($atts['module']) ? $atts['module'] : false;
			$styles = $atts['styles']['xl'];
			$bg_image_url = $this->get_bg_image_url($styles);
			...
			foreach ($animation['effects'] as $attribute => $value) {
				...
				if(strpos(strval($value), 'rem') !== false && $attribute != 'filter_drop-shadow') {
					...
					// is border width? check direction
					if($attribute == 'border-width') {
						$value = $this->get_border_width($styles, $value);
					}
				}
				// is rotate or skew?
				...
				} else if(strpos($attribute, 'reveal') !== false) {
					$reveal_props[$attribute] = $value;
			...
			// get reveal
			$reveal = $this->reveal($reveal_props, $atts['motions']['initial']);
			// add reveal to css
			if(false !== $reveal && !empty($reveal)) {
				$props['clipPath'] = $reveal;
			}
			// return props
			return $props;
		}
