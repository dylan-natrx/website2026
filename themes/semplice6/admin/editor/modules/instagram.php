...
if(!class_exists('sm_instagram')) {
	...
	class sm_instagram {
		...
		public function output_editor($values, $id) {
			...
			if(empty($instagram) && $media != 'curl' && true === $this->is_editor) {
				$this->output['html'] = semplice_module_placeholder('instagram', 'Once you connected your Instagram account your images will show up right here.', false, $this->is_editor, $id);
				...
				foreach ($media['data'] as $posts => $post) {
					if($index < $count) {
						if($post['media_type'] == 'IMAGE' || $post['media_type'] == 'CAROUSEL_ALBUM' || $post['media_type'] == 'VIDEO') {
							...
							// add thumb to holder if media url is available
				...
				$this->output['html'] = semplice_module_placeholder('instagram', 'Wrong or no access token.', false, $this->is_editor, $id);
			}
	...
	// instance
	editor_api::$modules['instagram'] = new sm_instagram;
}