...
if(!class_exists('sm_oembed')) {
	class sm_oembed {
		...
		public function output_editor($values, $id) {
			...
			// add placeholder
			$this->output['html'] = semplice_module_placeholder('oembed', false, false, true, $id);
		...
		public function output_frontend($values, $id) {
			...
			if(!empty($content) && false !== $content) {
				...
				$this->output['html'] = semplice_module_placeholder('oembed', false, false, false, $id);
			}
	...
	// instance
	editor_api::$modules['oembed'] = new sm_oembed;
}