...
if(!class_exists('sm_singleproject')) {
	class sm_singleproject {
		...
		public function output_editor($values, $id) {
			...
			// global hover
			$hover = semplice_customize('thumbhover');
			// is new?
			...
			if(isset($projects) && is_array($projects) && !empty($projects)) {
				...
				$output['html'] = '
					<div id="project-' . $project['post_id'] . '" class="is-content thumb' . $video_hover . '">
				...
				if(false === $this->is_editor) {
					$output['html'] = '<a href="' . $project['permalink'] . '" class="pg-link">' . $output['html'] . '</a>';
				}
			} else {
				$output['html'] = semplice_module_placeholder('singleproject', 'You either removed your project, need to publish it or haven\'t created one yet.', false, true, $id);
			}
			// output
	...
	// instance
	editor_api::$modules['singleproject'] = new sm_singleproject;
}