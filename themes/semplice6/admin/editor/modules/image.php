...
if(!class_exists('sm_image')) {
	class sm_image {
		...
		public function output_editor($values, $id) {
			...
			if(!empty($image_id)) {
				...
				$this->output['html'] = semplice_module_placeholder('image', false, true, $this->is_editor, $id);
			}
			...
			if(false === $this->is_editor) {
				if($lightbox == 'yes') {
					...
					if(!empty($link[$link['type']])) {
						return '<a class="image-link" href="' . get_permalink($link[$link['type']]) . '" target="' . $target . '">' . $image . '</a>';
					} else {
						return $image;
					}
				} else {
	...
	// instance
	editor_api::$modules['image'] = new sm_image;
}