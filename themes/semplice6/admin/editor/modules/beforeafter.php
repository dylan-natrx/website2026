...
if(!class_exists('sm_beforeafter')) {
		...
		public function output_editor($values, $id) {
			...
			if(true === $this->is_editor) {
				...
				if(!empty($before) && !empty($after)) {
					...
					$alt_before = semplice_get_image_alt($before);
					$alt_after = semplice_get_image_alt($after);
					$caption_before = wp_get_attachment_caption($before);
					$caption_after = wp_get_attachment_caption($after);
					// output
					$this->output['html'] = '
						<div id="' . $id . '-beforeafter" class="is-content">
							<img src="' . $images['before'][0] . '" data-label="Before" alt="' . $alt_before . '" caption="' . $caption_before . '" />
							<img src="' . $images['after'][0] . '" data-label="After" alt="' . $alt_after . '" caption="' . $caption_after . '" />
						</div>
						<script>
							(function($) {
					...
					// no images
					$this->output['html'] = semplice_module_placeholder('beforeafter', 'Please make sure to add both the before and after image in the content editor.', false, false, $id);
				}
			}
	...
	// instance
	editor_api::$modules['beforeafter'] = new sm_beforeafter;
}