...
if(!class_exists('sm_lottie')) {
		...
		public function output_editor($values, $id) {
			...
			if(true === $this->is_editor) {
				$this->output['html'] = '
					...
					' . semplice_module_placeholder('lottie', false, false, $this->is_editor, $id) . '
				';
			} else {
				// valid lottie
				$is_valid = false;
				...
				if(true === $is_valid) {
					...
					$this->output['html'] = semplice_module_placeholder('lottie', false, false, $this->is_editor, $id);
				}
			}
			// output
			return $this->output;
	...
	// instance
	editor_api::$modules['lottie'] = new sm_lottie;
}