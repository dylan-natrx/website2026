...
if(!class_exists('sm_video')) {
	class sm_video {
		...
		public function output_editor($values, $id) {
			...
			// get placeholder
			$this->output['html'] = semplice_module_placeholder('video', false, false, true, $id);
		...
		public function output_frontend($values, $id) {
			...
			// no controls for custom cursor
			$controls_class = ' ce-video-controls';
			if($hide_controls == 'on') {
				$controls_class = ' ce-video-no-controls';
			}
			...
			if(!empty($src)) {
				$this->output['html'] = '
					<div class="ce-video' . $controls_class . '" data-hide-controls="' . $hide_controls . '" style="width: 100%; max-width: 100%">
						<video class="video" ' . $video_dim . ' webkit-playsinline playsinline preload="metadata" ' . $poster . ' ' . $video_atts . '>
				...
				$this->output['html'] = semplice_module_placeholder('video', false, false, false, $id);
			}
	...
	// instance
	editor_api::$modules['video'] = new sm_video;
}