...
if(!class_exists('sm_portfoliogrid')) {
	...
	class sm_portfoliogrid {
		...
		public function output_editor($values, $id) {
			...
			if(!empty($projects)) {
				...
				$this->output['html'] = semplice_module_placeholder('portfoliogrid', 'Please note that only published projects are visible in the portfolio grid.', false, true, $id);
			}
	...
	// instance
	editor_api::$modules['portfoliogrid'] = new sm_portfoliogrid;
}