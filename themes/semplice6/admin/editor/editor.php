...
class editor {
	...
	public function get_content($ram, $visibility, $is_block, $is_coverslider) {
		...
		if(true === $is_coverslider) {
			...
			// is posts filter defined?
			if(!isset($ram['posts_filter'])) {
				$ram['posts_filter'] = false;
			}
			// get coverslider
			$coverslider = semplice_get_coverslider($ram['coverslider'], $visibility, $script_execution, $ram['posts_filter']);
			// add to output
			$output = array(
			...
			foreach($ram['order'] as $section_id => $section) {
				...
				if($section_id != 'cover' || $visibility == 'editor' || $this->has_cover($section_id, $ram['visibility'], $is_cover, $cover_visibility)) {
					...
					// add to images (für das context menu update benötigt da nun keine css styles mehr im post css sind und somit bilder fehlen würden die z.b. bei blocks oder import cover nicht registriert wurden früher für column inner styles und row container styles)
					$output['images'] = $output['images'] + $section_output['images'];
	...
	public function navigator() {
		...
		<div class="inner">
			...
			<h3 class="pages-projects-heading"><span class="left">Pages</span><span class="right">Projects</span></h3>
			<div class="posts">
				<div class="pages">
					<ul>';
							// list projects
							...
							if($pages) {
								foreach($pages as $page) {
									// truncate title	
									$title_truncated = (strlen($page->post_title) > 24) ? mb_substr($page->post_title, 0, 24) . '...' : $page->post_title;
									// output list element					
					...
					<ul>';
							...
							if($projects) {
								foreach($projects as $project) {
									// truncate title	
									$title_truncated = (strlen($project->post_title) > 24) ? mb_substr($project->post_title, 0, 24) . '...' : $project->post_title;
									// output list element									
