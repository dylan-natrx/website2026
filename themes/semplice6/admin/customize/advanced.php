		...
		public function generate_css($is_frontend) {
			...
			if(is_array($advanced)) {
				...
				if($is_frontend) {
					...
					// progress bar height
					if(isset($advanced['progress_bar_height'])) {
						$advanced_css .= '#nprogress .bar { height: ' . $advanced['progress_bar_height'] . '; }';
					}	

					// top arrow visibility
					if(isset($advanced['top_arrow_visibility']) && $advanced['top_arrow_visibility'] == 'hidden') {
						$advanced_css .= '.back-to-top { display: none !important; }';
					}
					...
					if(isset($advanced['lightbox_bg_color'])) {
						...
						// transparent top bar
						$advanced_css .= '.pswp__top-bar, .pswp__button--arrow--left:before, .pswp__button--arrow--right:before { background-color: rgba(' . $bg_color['r'] . ', ' . $bg_color['g'] . ', ' . $bg_color['b'] . ', .15) !important; }';
					}
