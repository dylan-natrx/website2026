...
class admin_api {
	...
	public function __construct() {
		...
		// customize list
		$customize_settings = array('grid', 'webfonts', 'navigations', 'typography', 'thumbhover', 'projectnav', 'transitions', 'footer', 'blog', 'advanced', 'cursor', 'ppc');
	...
	public function register_routes() {
		...
		$routes = array(
			...
			'/edit-wp-template/(?P<id>\w+)' => array('readable', 'edit_wp_template'),
	...
	public function dashboard($request) {
		...
		$news_content = wp_remote_get('https://news.semplice.com/wp-json/news/v1/news_six');
	...
	public function init_editor($request) {
		...
		$css = array(
			...
			'thumbhover'	=> admin_api::$customize['thumbhover']->global_thumbhover_css(true)
		);
		...
		if(!empty($ram)) {
			...
			if(is_array($ram['images'])) {
				...
				foreach ($ram['images'] as $image_id => $image_url) {
					// add image
					$images[$image_id] = semplice_image_with_sizes($image_id);
				}
			}
	...
	// init template edit
	public function edit_wp_template($request) {

		// post meta
		$template_id = $request['post_id'];

		// get content
		$content_json = get_option('semplice_template_' . $template_id);

		// init masterblocks or get default template
		if(false !== $content_json && !empty($content_json)) {
			$content_json = $this->editor->init_masterblocks($content_json);
		} else {
			$content_json = file_get_contents(get_template_directory() . '/assets/json/wp-templates/' . $request['post_id'] . '.json');
		}

		// get post settings from post meta
		$template_settings = json_decode(get_option('semplice_template_' . $template_id . '_settings'), true);

		// generate post settings
		$template_settings = semplice_generate_template_settings($template_settings);
		...
		if(!empty($ram)) {
			...
			// get content
			$content = $this->editor->get_content($ram, 'editor', false, false);
		...
		$output = array(
			...
			'images'  	 		=> !empty($images) ? $images : '',
			'meta'				=> false,
			'post_settings' 	=> $template_settings,
			'navigator'			=> $this->editor->navigator(),
			'post_select'		=> $post_select,
			'covers'			=> $covers,
			'is_coverslider'	=> false,
			'posts'				=> semplice_get_apg_posts('content', false),
			'post_revision'  	=> 'latest_version',
			'template_dropdown' => semplice_get_template_dropdown(),
		);
