...
if(!is_admin()) {
	...
	function semplice_get_content() {
		...
		if(isset($advanced['exclusive_navbar']) && $advanced['exclusive_navbar'] != 'default' && false === semplice_is_removed($advanced['exclusive_navbar'])) {
			$semplice_content['navbar'] = admin_api::$customize['navigations']->get('both', $advanced['exclusive_navbar'], false, true);
		} else if(!isset($semplice_content['navbar'])) {
...
function frontend_css() {
	...
	// navigations css
	$exclusive_nav = '';
	$navigations_css = str_replace($search, $replace, $semplice_custom_css->navigation());

	// is exclusive nav?
	if(isset($advanced['exclusive_navbar']) && $advanced['exclusive_navbar'] != 'default' && false === semplice_is_removed($advanced['exclusive_navbar'])) {
		$exclusive_nav = $navigations_css;
		$navigations_css = '';
	}
...
function semplice_localize_script_defaults() {
	...
	$output = array(
		...
		'exclusive_nav'			=> semplice_has_exclusive_nav(),
		'customize'				=> array(
