...
function semplice_get_coverslider($coverslider, $visibility, $script_execution, $posts_filter) {
	// output
	$output = array(
		'html' 		=> '',
		'css'  		=> '',
	...
	if(!empty($posts_with_covers)) {
		...
		if(!empty($valid_covers)) {
			...
			// get output
			$output = semplice_coverslider_output($coverslider, $visibility, $script_execution, $posts_filter);
		} else {
...
function semplice_coverslider_output($coverslider, $visibility, $script_execution, $posts_filter) {
	// globals
	global $detect;
	global $editor_api;
	...
	if(null !== $ram && isset($ram['cover'])) {
		...
		if(is_array($cover_styles)) {
			foreach ($cover_styles as $bp => $bp_styles) {
				...
				if(isset($bp_styles['background-image'])) {
					$output['images'][$bp_styles['background-image']] = semplice_image_with_sizes($bp_styles['background-image']);
				}
				// video fallback
				if(isset($bp_styles['bg_video_fallback'])) {
					$output['images'][$bp_styles['bg_video_fallback']] = semplice_image_with_sizes($bp_styles['bg_video_fallback']);
				}
			}
		}
		// cover iterate
		...
		foreach ($cover_iterate as $row_id => $columns) {
			...
			foreach ($columns['columns'] as $column_id => $column_content) {
				...
				if(isset($ram[$column_id]['styles']['xl']['background-image'])) {
					$output['images'][$ram[$column_id]['styles']['xl']['background-image']] = semplice_image_with_sizes($ram[$column_id]['styles']['xl']['background-image']);
				}
				// get content wrapper bg image and add to images_array
				if(isset($ram[$column_id]['wrapperStyles']) && isset($ram[$column_id]['wrapperStyles']['xl']['background-image'])) {
					$output['images'][$ram[$column_id]['wrapperStyles']['xl']['background-image']] = semplice_image_with_sizes($ram[$column_id]['wrapperStyles']['xl']['background-image']);
				}
				// create new id
				$new_column_id = 'column_' . substr(md5(rand()), 0, 9);
				...
				foreach ($column_content as $content_id) {
					// get background image and add to images_array
					if(isset($ram[$content_id]['styles']['xl']['background-image'])) {
						$output['images'][$ram[$content_id]['styles']['xl']['background-image']] = semplice_image_with_sizes($ram[$content_id]['styles']['xl']['background-image']);
					}
					// create new id
					$new_content_id = 'content_' . substr(md5(rand()), 0, 9);
					// add to ram
