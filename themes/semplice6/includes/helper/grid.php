...
function semplice_get_grid_breakpoint($gutter, $mode, $is_desktop) {
	...
	if($mode == 'editor') {
		...
		// column
		$output .= $prefix . ' .column, .grid-column, .single-post-column {
		...
		$output .= '.column, .grid-column, .single-post-column {
			padding-left: ' . ($gutter / 2) . 'px;
			padding-right: ' . ($gutter / 2) . 'px;
		}';
...
function semplice_project_nav_html($mode, $is_frontend, $post_id) {
	...
	// get project panel
	$projectnav = semplice_customize('projectpanel');
							...
							<div class="column" data-xl-width="12">
								<div class="np-inner' . $np_classes . '" data-np-justify="' . $np_justify . '" data-np-alignment="' . $np_alignment . '" data-np-image-scale="' . $np_image_scale . '" data-np-next-only="' . $np_next_only . '">
									<a class="semplice-prev np-link" href="' . get_permalink($prev) . '">
										...
										' . semplice_get_project_nav_label('prev', $np_arrow, $np_label, $prev_label, $prev_type, $np_mouseover_title_fade, $np_font, $np_font_sub) . '
									</a>
									<a class="semplice-next np-link" href="' . get_permalink($next) . '">
										<div class="np-bg"' . $background_next . '></div>
										' . semplice_get_project_nav_label('next', $np_arrow, $np_label, $next_label, $next_type, $np_mouseover_title_fade, $np_font, $np_font_sub) . '
									</a>
								</div>
							</div>
						</div>
...
function semplice_get_project_nav_label($state, $arrow, $label, $title, $type, $fade, $font, $font_sub) {
	// arrow
