...
function semplice_update_check() {
	...
	} else if(semplice_theme('edition') == 'single' && isset($license['product']) && $license['product'] != 's6-single') {
		$meta_data_url = 'https://update.semplice.com/update_s6_studio.json';
	} else {
		// check license and theme folder
		$meta_data_url = 'https://update.semplice.com/update_s6_' . semplice_theme('edition') . '.json';
	}
	...
	if(false !== $meta_data_url) {
		...
		if($theme_folder == 'semplice6' && is_array($license) && true === $license['is_valid']) {
			// make sure its s6
			if(strpos($license['product'], 's6') !== false || $license['product'] == 'subscription') {
	...
	if(is_array($theme_updates)) {
		foreach ($theme_updates as $theme => $meta) {
			...
			if($meta->Name == 'Semplice v6') {
				...
				if(semplice_theme('edition') == 'single' && $license['product'] != 's6-single') {
