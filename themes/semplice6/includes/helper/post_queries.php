		...
		$header = '
			<div class="admin-row posts-header">
				<div class="sub-header admin-column">
					<h2 class="admin-title">All ' . ucfirst($request['post_type']) . 's
...
function semplice_post_row($post_id, $post_type, $post_title, $post_status, $is_duplicate, $thumbnail, $is_semplice, $slug, $projects_view) {
	...
	if(empty($post_title)) {
		...
		$post_title = (strlen($post_title) > $lenght) ? mb_substr($post_title, 0, $lenght) . '...' : $post_title;
	}
	...
	if(is_array($posts)) {
		foreach ($posts as $post) {
			...
			if(false === $is_coverslider && true === $is_semplice) {
				$output .= '<option value="' . $post->ID . '">' . substr($post->post_title, 0, 50) . '</option>';
			}
		}
		return $output;
	} else {
...
// -----------------------------------------
// get blogposts checklist
// -----------------------------------------

function semplice_blogposts_checklist() {
	// get pages
	$posts = get_posts(array('posts_per_page' => -1, 'post_type' => array('post')));
	// output
	$output = '';
	// iterate pages object
	if(is_array($posts)) {
		foreach ($posts as $post) {
			$output .= '<li id="post-' . $post->ID . '"><label class="selectit"><input value="' . $post->ID . '" type="checkbox" name="post_id[]" id="post-id-' . $post->ID . '"/>' . $post->post_title . '</label></li>';
		}
	}
	// return
	return $output;
