...
// include helpers
$helpers = array('post_queries', 'blog', 'images', 'videos', 'thumbnails', 'onboarding', 'grid', 'typography', 'navigation', 'sharebox', 'styles', 'covers', 'licensing', 'notices');
...
function semplice_show_content($id, $what) {
	...
	// check if an exclusive nav is set
	if(semplice_has_exclusive_nav() == 'enabled') {
		$header = $semplice_content['navbar']['html'] . '<div id="content-holder" data-active-post="' . $id . '">';
	} else {
		$header = '<div id="content-holder" data-active-post="' . $id . '">' . $semplice_content['navbar']['html'];
	}
...
function semplice_generate_ram_ids($ram, $is_encoded, $is_block) {
	...
	// images array
	$images = '';
	$image_modules = array('image', 'gallerygrid', 'video', 'gallery', 'marquee');
	...
	foreach ($ram['order'] as $section_id => $section) {
		...
		if(isset($ram[$section_id]) && isset($ram[$section_id]['masterblock']) && strpos($ram[$section_id]['masterblock'], 'master_') !== false) {
			...
			if(isset($ram[$section_id]) && $section_id != 'cover') {
				...
				// get background image for container setyles and add to array
				if(isset($ram[$section_id]['containerStyles'])) {
					$images .= semplice_get_background_image($ram[$section_id]['containerStyles']['xl']);
				}
				...
				foreach($section_iterate as $row_id => $columns) {
					...
					foreach ($columns['columns'] as $column_id => $column_content) {
						...
						// get content wrapper bg image and add to images_array
						if(isset($ram[$column_id]['wrapperStyles'])) {
							$images .= semplice_get_background_image($ram[$column_id]['wrapperStyles']['xl']);
						}
...
function semplice_blocks_image_array($images) {
	...
	if(!empty($images)) {
		...
		foreach ($images as $image_id) {
			// get image
			$images_arr[$image_id] = semplice_image_with_sizes($image_id);
		}
	} else {
		$images_arr = 'noimages';
	}
...
function semplice_generate_template_settings($settings) {
...
function semplice_get_sr_status() {
	...
	if(is_object($post)) {
		...
		// check sr status
		if($post_id == 'posts' && false === $custom_blog || $post->post_type == 'post' && false === $custom_blog) {
			// is array?
			$sr_status = semplice_get_blog_sr_status();
		} else if(isset($semplice_content['branding']) && isset($semplice_content['branding']['scroll_reveal']) && $semplice_content['branding']['scroll_reveal'] == 'disabled' || $post_id == 'notfound' || !$is_semplice && false === $custom_blog) {
			$sr_status = 'disabled';
		}
	}
...
function semplice_custom_cursor($mode) {

	// detect mobile class
	global $detect;

	// frontend settings
	$settings = semplice_customize('cursor');

	// cursor
	$cursor = array('status' => false, 'classes' => 'semplice-cursor');

	// check if mode option in the admin is already set
	if(false === $detect->isMobile() && false !== $settings && isset($settings['custom_cursor']) && $settings['custom_cursor'] == 'enabled') {
		if($mode == 'output') {
			echo '
				<div id="semplice-cursor">
					<div class="semplice-cursor-inner">
						<span class="cursor-text cursor-view">View</span>
						<span class="cursor-icon cursor-top-arrow">' . get_svg('frontend', 'icons/cursor_arrow_top') . '</span>
						<span class="cursor-icon cursor-bottom-arrow">' . get_svg('frontend', 'icons/cursor_arrow_bottom') . '</span>
						<span class="cursor-icon cursor-right-arrow">' . get_svg('frontend', 'icons/cursor_arrow_right') . '</span>
						<span class="cursor-icon cursor-left-arrow">' . get_svg('frontend', 'icons/cursor_arrow_left') . '</span>
						<span class="cursor-icon cursor-zoom-in">' . get_svg('frontend', 'icons/cursor_zoom_in') . '</span>
						<span class="cursor-icon cursor-ba">' . get_svg('frontend', 'icons/cursor_ba') . '</span>
						<span class="cursor-icon cursor-drag">' . get_svg('frontend', 'icons/cursor_drag') . '</span>
					</div>
				</div>
			';
		} else {
...
function semplice_get_breakpoints($has_xl = false) {
	$breakpoints = array(
		'xl' => array(
			'min' => ' and (min-width: 1170px)',
			'max' => ''
		),
		'lg' => array(
...
function semplice_get_modules() {
	...
	foreach ($modules as $groups => $group) {
		// label
		$list .= '
			<h3 class="group-label inter_regular"><span>' . $group['title'] . '</span></h3>
		...
		foreach ($group['modules'] as $module => $content) {
			...
			if(semplice_theme('edition') == 'single' && in_array($module, $studio_modules)) {
				$list .= '<li class="module-' . $module . '"><a class="add-content-single admin-click-handler" data-module="' . $module . '" data-handler="execute" data-action="studioFeatures" data-action-type="popup" data-feature="' . $module . '"><span class="icon">' . get_svg('backend', 'icons/module_' . $module) . '</span><span>' . $content . '</span><i>Studio Edition</i></a></li>';
			} else {
				$list .= '<li class="module-' . $module . '"><a class="add-content add-module' . $classes . '" data-module="' . $module . '"><span class="icon">' . get_svg('backend', 'icons/module_' . $module) . '</span><span>' . $content . '</span></a></li>';
			}
		}
		$list .= '</ul>';
	}
...
function semplice_about() {
	...
	$licenses = array(
		's6-single'				=> 'Single',
		's6-studio'				=> 'Studio',
		's6-single-to-studio'	=> 'Studio',
		's6-business'			=> 'Business',
		's6-single-to-business'	=> 'Business',
		's6-studio-to-business'	=> 'Business',
	...
	return '
		...
		<p class="about-update">
			...
			<span class="new-version">6.0</span>
...
function semplice_get_transitions_preloader() {
	...
	$transitions = semplice_customize('transitions');
...
function semplice_sml_get_attachment($attachment) {
	...
	if(strpos($attachment->post_mime_type, 'image') !== false) {
		...
		// thumbnail
		$thumbnail = '<img src="' . $image_urls['medium_large'] . '" onerror="this.onerror=null; this.src=\'' . get_template_directory_uri() . '/assets/images/admin/missing_image_' . semplice_get_backend_style() . '.png\'">';
		// add class
		$classes = 'sml-image';
	} else {
		// url
...
function semplice_module_placeholder($module, $description, $has_upload, $is_editor, $id) {
	...
	if($module == 'blogposts' && true === $is_editor) {
		$blogposts_connect = '<div class="mp-connect"><a class="oauth-connect no-ep add-blogposts semplice-button white-button" href="' . admin_url('edit.php') . '" target="_blank">Add Blogposts</a><a class="oauth no-ep semplice-button regenerate-blogposts" data-content-id="' . $id . '">Refresh preview</a></div>';
	}
	...
	if(true === $has_upload) {
		...
		// add video support for gallery
		if($module == 'gallery') {
			$dropzone .= ' video-support';
		}
		// upload
		$upload = '
			<div class="mp-upload">
		...
		<div id="studio-feature-' . $feature . '" class="popup studio-popup">
			...
			<div class="popup-inner feature-popup-inner">
				...
				<div class="popup-footer">
					' . $see_it_live . '
					<a href="https://transactions.sendowl.com/products/78691114/20846C22/add_to_cart" class="feature-purchase" target="_blank">Upgrade to Studio for $39</a>
				</div>					
			</div>
		</div>
	';
...
function semplice_get_module_svgs() {
	...
	// modules
	$modules = array('paragraph', 'text', 'fluidtext', 'image', 'gallery', 'video', 'spacer', 'button', 'oembed', 'portfoliogrid', 'singleproject', 'advancedportfoliogrid', 'code', 'share', 'socialprofiles', 'dribbble', 'instagram', 'gallerygrid', 'mailchimp', 'beforeafter', 'lottie', 'blogposts', 'blogarchives', 'blogsearch', 'blogcomments', 'marquee', 'accordion');
...
function semplice_back_to_top_arrow() {
	...
	// align
	if(isset($advanced['top_arrow_align'])) {
		$align = $advanced['top_arrow_align'];
	}
	// return
	echo '
		<div class="back-to-top" data-arrow-align="' . $align . '">
			<a class="semplice-event" data-event-type="helper" data-event="scrollToTop">' . $arrow . '</a>
		</div>
	';
}
...
function semplice_get_animate_presets() {
	...
	$preview = file_get_contents(get_template_directory() . '/assets/json/animate/preview.json');
	// get json for premade presets
	$premade = file_get_contents(get_template_directory() . '/assets/json/animate/presets.json');
	// add to presets
	$presets = array(
			...
			<div class="popup-inner">
				...
				<div class="popup-content whats-new-content">
					...
					<div class="changelog">
						' . $changelog_head . '
						' . $changelog_output . '
						<div class="changelog-link"><a href="https://www.semplice.com/changelog-v6-' . semplice_theme('edition') . '" target="_blank">View complete changelog on semplice.com</a></div>
					</div>
				</div>				
			</div>
		</div>
...
// -----------------------------------------
// get gutenberg options
// -----------------------------------------

function semplice_gutenberg_options() {
	// output
	$options = array(
		'customStyles' => false
	);
	// typography
	$typography = semplice_customize('typography');
	if(false !== $typography && !empty($typography['custom'])) {
...
// -----------------------------------------
// check if a post is not deleted or in trash
// -----------------------------------------

function semplice_is_removed($id) {
	$is_removed = true;
	// is post or navbar?
	if(strpos($id, 'nav_') !== false) {
		// get navs
		$navigations = semplice_customize('navigations');
		if(is_array($navigations) && isset($navigations[$id])) {
			$is_removed = false;
		}
	} else if(false !== get_post_status($id) && get_post_status($id) != 'trash') {
		return false;
	}
	// return
	return $is_removed;
...
function semplice_has_exclusive_nav() {
	// customize advanced
	$advanced = semplice_customize('advanced');

	// check if an exclusive nav is set
	if(isset($advanced['exclusive_navbar']) && $advanced['exclusive_navbar'] != 'default' && false === semplice_is_removed($advanced['exclusive_navbar'])) {
		return 'enabled';
	} else {
		return 'disabled';
	}
